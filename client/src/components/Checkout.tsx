...
function CheckoutPage()
{
   ...
   */
   ...
   const cartTaxes = 5.00 // cartTotalPrice * 0.0825
   ...
   const placeOrder =  async (customerForm: CustomerForm) =>  {
      ...
      const url = `http://localhost:8080/JenaBookstoreReactOrder/api/orders`; //'api/orders'; //webdev.cs.vt.edu
      ...
      switch (name) {
         case 'name':
            ...
            if(value.length < 4) {
               ...
               setNameError("Name must be shorter than 45 characters!");
            }
            else {
                   setNameError("");
                 }
         ...
         case 'address':
            ...
            if(value.length < 4) {
               ...
               setAddressError("Address must be shorter than 45 characters!");
            }
            else {
               setAddressError("");
            }
  ...
  // TO DO submitOrder function comes here. See the project Spec
   async function submitOrder(event:FormEvent) {
      ...
      if (!isFormCorrect) {
         ...
         if (formData.name.length === 0) setNameError("Name is required.");
         if (formData.address.length === 0) setAddressError("Address is required.");
         if (formData.phone.length === 0) setPhoneError("Phone number is required.");
         if (formData.email.length === 0) setEmailError("Email is required.");
         if (formData.ccNumber.length === 0) setCCNumberError("Credit card number required.");
      } else {
       ...
       <>{cart.length === 0 ?
           <div className={"page-empty-state"}>
              <div className="header-text">
                 <h1>Fill your shopping cart to checkout again.</h1>
              </div>
           </div>
              :
                                       ...
                                       <section className="checkoutStatusBox">
                                          {(checkoutStatus === 'ERROR') ?
                                              <div style={{color: 'var(--error-red-color)'}}>
                                                 Error: Please fix the problems above and try again. All fields are
                                                 required.
